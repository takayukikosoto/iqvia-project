...
create policy "events_write_own" on public.events for insert with check (owner_user_id = auth.uid());
...
create or replace function public.set_events_updated_at() returns trigger language plpgsql as $$
...
create index if not exists idx_events_owner_start_end on public.events (owner_user_id, start, "end");
